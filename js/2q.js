...
(() => {
  ...
  const q = window[window.QConfig.q.functionName] = function () {
    ...
    Object.entries(Object.assign(args.blueprint.attributes ?? {}, args.blueprint.attr ?? {}))
      .forEach(attribute => qel.setAttribute(...attribute));
    ...
    // Turn strings in children into text nodes
    for (const i in args.children) {
      if (typeof args.children[i] === 'string') args.children[i] = document.createTextNode(args.children[i]);
    }
    ...
    if (window.QConfig.qget && window.QConfig.qget.enabled) {
      ...
      qel[varns + 'names'] = {};
      if (args.qname || args.blueprint.qn || args.blueprint.named) {
